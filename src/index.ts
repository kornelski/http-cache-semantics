...
export default class CachePolicy {
  ...
  constructor(
    ...
    this.#cacheHeuristic = undefined !== cacheHeuristic ? cacheHeuristic : 0.1; // 10% matches IE
  ...
  timeToLive() {
    ...
    const staleIfErrorAge =
      age + toNumberOrZero(this.#resCacheControl['stale-if-error'] as string);
    const staleWhileRevalidateAge =
      age +
      toNumberOrZero(this.#resCacheControl['stale-while-revalidate'] as string);
    return Math.round(
    ...
    if (this.#resHeaders.has('etag')) {
      headers.set(
        ...
        headers.has('if-none-match')
          ? `${headers.get('if-none-match')}, ${this.#resHeaders.get('etag')}`
